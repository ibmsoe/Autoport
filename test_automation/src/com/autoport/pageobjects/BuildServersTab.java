...
public class BuildServersTab {
	...
	public void verifyJenkinsStatus(){
		...
		int totalJobs = noOfSuccessJobs + noOfUnstableJobs + noOfFailingJobs + noOfDisabledJobs;
	...
	public String selectTwoVersionsToInstall(String packagename){
		...
		List<WebElement> pkgCheckbox  =	driver.findElements(By.xpath("//table[@id='singleServerPackageListTable']/tbody/tr/td[2][text()[contains(.,'" + packagename +"')]]//ancestor::tr[1]/td/input"));
		
		String firstVersion = driver.findElement(By.xpath("//table[@id='singleServerPackageListTable']/tbody/tr/td[2][text()[contains(.,'apache-ant')]]//ancestor::tr[1]/td[4]")).getText();
	...
	public String selectPackageToUpdate() throws InterruptedException{
		...
		WebElement packagename = driver.findElement(By.xpath("//table[@id='singleServerPackageListTable']/tbody/tr/td[5][text()[contains(.,'Yes')]]/preceding-sibling::td[3]"));;
	...
	public void selectPackageToUninstall(){
		...
		driver.findElement(By.xpath("//table[@id='singleServerPackageListTable']/tbody/tr/td[1]/input")).click();
	...
	public void verifyInstallationSuccessPopUp(String packagename){
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='errorAlert']//tbody/tr[2]")));
	...
	public void verifyInstalledVersionIsNA(String packagename){
		...
		if(version.contains("N/A") ){
			LogResult.pass("Installed version is displayed as N/A for installed package");
		}
		else{			
			LogResult.fail("Installed version for package "+ packagename + " is " + version);
		}
	}
	
	/* Function to verify that the package updation success pop up is displayed correctly */
	public void verifyUpdationSuccessPopUp(String packagename){
		...
		if(installedPackageName.contains(packagename))
			LogResult.pass("Package name is correct in updation success message");		
		else
			LogResult.fail("Package name is not correct in updation success message");
		
		if(actionname.contains("install/update"))
			LogResult.pass("Action name is correct in updation success message");		
		else
			LogResult.fail("Action name is not correct in updation success message");
	...
	/* Function to verify that the package is updated successfully and Installed version and available versions are same*/
	public void VerifyPackageUpdateIsSuccessfull(String packagename){
		
		String installedversion =  driver.findElement(By.xpath("//table[@id='singleServerPackageListTable']/tbody/tr/td[text() = '" + packagename + "']/following-sibling::td[1]")).getText();
		
		String availableversion =  driver.findElement(By.xpath("//table[@id='singleServerPackageListTable']/tbody/tr/td[text() = '" + packagename + "']/following-sibling::td[2]")).getText();
		
		if(installedversion.equals(availableversion)){
			LogResult.pass("Installed version and availabble versions are same and package is updated.");
		}
		else{			
			LogResult.fail("Installed version and availabble versions are not same and package is not updated.");
		}
	}
	
	/* Function to verify that the package uninstallation success pop up is displayed correctly */
	public void verifyUninstallationSuccessPopUp(String packagename){
		...
		if(installedPackageName.contains(packagename))
			LogResult.pass("Package name is correct in uninstallaton success message");		
		else
			LogResult.fail("Package name is not correct in uninstallaton success message");
		
		if(actionname.contains("remove/update"))
			LogResult.pass("Action name is correct in uninstallaton success message");		
		else
			LogResult.fail("Action name is not correct in uninstallaton success message");
		
		if(status.contains("SUCCESS"))
			LogResult.pass("Status is correct in uninstallaton success message");		
		else
			LogResult.fail("Status is not correct in uninstallaton success message");		
	...
	/* Function to verify that the package is not installed pop up message is displayed when unstalling it */
	public void verifyPkgNotInstalledMessage(){
		
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='errorAlert']/div/div/div[1]")));
		String alreadyInstalledMsg = driver.findElement(By.xpath("//div[@id='errorAlert']/div/div/div[1]")).getText();
		
		if(alreadyInstalledMsg.contains("The selected packages are not installed and therefore cannot be removed!"))
			LogResult.pass("The selected packages are not installed and therefore cannot be removed message is displayed correctly");		
		else
			LogResult.fail("The selected packages are not installed and therefore cannot be removed message is not displayed correctly");	
	...
	/*Function to enter packagename to search */
	public void enterPackageToSearchUsingManagedServices(String packageName) throws InterruptedException{
	...
	/* Function to click on List RHEL button */
	public void clickListRhelBtn(){
		
	listRHELBtn.click();
		
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("multiServerPackageListTable")));
	...
	/* Function to click on List Ubuntu button */
	public void clickListUbuntuBtn(){
	...
	/* Function to click on List All button */
	...
	/* Function to select maximum records to display */
	public void selectMaxRecordsToDisplayForManagedServices() throws InterruptedException{
		
		driver.findElement(By.xpath("//div[@id='manageRuntime']/div[1]//span[@class='page-list']/span/button")).click();
		Thread.sleep(1000);
		
		driver.findElement(By.xpath("//div[@id='manageRuntime']/div[1]//span[@class='page-list']/span/ul/li[last()]/a")).click();
		Thread.sleep(5000);
	}
		
	/* Function to verify that correct notification message is displayed above search results table for managed servics*/
	public void verifyNotificationMessageForManagedServices(String message){
		
		String notificationMsg = driver.findElement(By.xpath("//div[@id='manageMultipleSlavePanel']/div[3]/div[2]/div")).getText();
	...
	/*Function to verify that the build servers are populated as per list button selected */
	public void verifyPopulatedBuildServers(String machineType){
		
		List<WebElement> buildservers = driver.findElements(By.xpath("//div[@id='manageMultipleSlavePanel']//label[@class='checkbox']/input"));
		...
		switch (machineType)
        {
            case "RHEL":
            	for (WebElement buildserver : buildservers){       			
        			...
        			if (strBS.contains("rhel")){
        				LogResult.pass(strBS + " Build servers are displayed correctly");
        			}
        			else{
        				LogResult.fail(strBS + " Build servers are not displayed correctly ");
        				break;
        			}
        		}
                break;
            ...
            case "All":
            	for (WebElement buildserver : buildservers){       			
        			String strBS = buildserver.getAttribute("value");
        			if (strBS.contains("Ubuntu") || strBS.contains("RHEL")){
        				LogResult.pass(strBS + " Build server is displayed ");
        			}
        			else{
        				LogResult.fail(strBS + " Build server is displayed ");
	...
	/*Function to verify that the search results are correct for in multi server package list table*/
	public void verifySearchResultsForManagedServicesList(String machineType){
		
		List<WebElement> oss = driver.findElements(By.xpath("//table[@id='multiServerPackageListTable']/tbody/tr/td[7]"));
		
		List<WebElement> nodes = driver.findElements(By.xpath("//table[@id='multiServerPackageListTable']/tbody/tr/td[8]"));
		...
		switch (machineType)
        {
            case "RHEL":
            	for (WebElement os : oss){       			
        			...
        			if (osname.contains("RHEL")){        				
        			}
        			else{
        				LogResult.fail(osname + " OS is not rhel");
        				break;
        			}
        		}
            	...
            	for (WebElement node : nodes){       			
        			...
        			if (nodename.contains("rhel")){        				
        			}
        			else{
        				LogResult.fail(nodename + " Node is not rhel");
        				break;
        			}
        		}
                break;
            ...
            case "All":
            	for (WebElement os : oss){       			
        			String osname = os.getText();
        			if (osname.contains("Ubuntu") || osname.contains("RHEL")){        				
        			}
        			else{
        				LogResult.fail(osname + " OS is not Ubuntu");
        				break;
            	...
            	for (WebElement node : nodes){       			
        			String nodename = node.getText();
        			if (nodename.contains("ubuntu") || nodename.contains("rhel")){        				
        			}
        			else{
        				LogResult.fail(nodename + " Node is not Ubuntu");
        				break;
	...
	/* Function to verify that the checkboxes are not enabled for non removable packages*/	
	public void verifyChkboxNotEnabledForNonRemovablePkgs(){
		
		List<WebElement> nonRemovablePkgsChkBox = driver.findElements(By.xpath("//table[@id='multiServerPackageListTable']/tbody/tr/td[6][text()='No']/ancestor::tr[1]/td[1]"));
		
		
	}
	
	/* Function to select the package to add or remove to/from manages services of list */
	public String selectPackageToAddRemove(String packagename){
		
		WebElement packageChkBox =  driver.findElement(By.xpath("//table[@id='multiServerPackageListTable']/tbody/tr/td[2][text()[contains(.,'" + packagename + "')]]/ancestor::tr/td[1]/input"));
	...
	public void verifyAndAcceptAddPopup(String packageName, String version) throws InterruptedException{
		...
		if(alertmsg.contains(version) && alertmsg.contains(packageName) && alertmsg.contains("The below packages are eligible for Add")){
			LogResult.pass("Correct Pop up message stating the eligible package and version to add is displayed.");
		alert.accept();
		}
		else{
	...
	/* Function to verify that the package is Added to managed list successfully message is displayed correctly. */
	public void verifyPkgAddSuccessMessage(){
		...
		if(alreadyInstalledMsg.contains("Success!"))
			LogResult.pass("Added Successfully ! message is displayed correctly");		
		else
			LogResult.fail("Added Successfully ! message is not displayed correctly");	
		
		driver.findElement(By.xpath("//div[@id='errorAlert']/div/div/div[3]/button")).click();
	}
	
	/* Function to verify that the package is removed from managed list successfully message is displayed correctly. */
	public void verifyPkgRemoveSuccessMessage(){
		...
		if(alreadyInstalledMsg.contains("Success!"))
			LogResult.pass("Removed Successfully ! message is displayed correctly");		
		else
			LogResult.fail("Removed Successfully ! message is not displayed correctly");	
	...
	/* Function to verify the Please select build server message is displayed correctly. */
	public void verifySelectBuildServerMsg(){
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='errorAlert']/div/div/div[1]")));
		String selectBuildServermsg = driver.findElement(By.xpath("//div[@id='errorAlert']/div/div/div[1]")).getText();
		
		if(selectBuildServermsg.contains("Please select build server"))
			LogResult.pass("Please select build server message is displayed correctly");		
		else
			LogResult.fail("Please select build server message is not displayed correctly");	
	...
	public void selectBuildServerToSynch(String buildserver){
		...
		String xpathOfBuildserver = "//div[@id='manageMultipleSlavePanel']//label[@class='checkbox']/input[contains(@value,'" + buildserver +"')]";
		WebElement BScheckbox =  driver.findElement(By.xpath(xpathOfBuildserver));
		BScheckbox.click();
	...
	/* Function to verify that Installation initiated in background pop message is displayed corretly*/
	public void verifyBackgroundInstallationPopup(String buildServer){
		
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='errorAlert']/div/div/div[1]")));
		String Msg = driver.findElement(By.xpath("//div[@id='errorAlert']/div/div/div[1]")).getText();
		...
		if(Msg.contains(str1) && Msg.contains(str2) && Msg.contains(str3) && Msg.contains(str4) && Msg.contains(buildServer))
			LogResult.pass("Backgroud installation and uninstallation message is displayed correctly");		
		else
			LogResult.fail("Backgroud installation and uninstallation message is not displayed correctly");	
	...
	public void enterPackageToUpload(String packagename) throws InterruptedException{
	...
	/* Function to verify that the package is uploaded successfully message is displayed correctly. */
	public void verifyUploadedSuccessfullyMessage(){
		...
		if(alreadyInstalledMsg.contains("Uploaded Successfully !"))
			LogResult.pass("Uploaded Successfully ! message is displayed correctly");		
		else
			LogResult.fail("Uploaded Successfully ! message is not displayed correctly");	

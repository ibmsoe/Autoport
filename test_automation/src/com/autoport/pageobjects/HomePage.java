...
public class HomePage {
    ...
    WebElement sftpHostLabel;

    @FindBy(id = "hostname")
    WebElement sftpHostTxtBx;
    ...
    public void verifySettingsUI(String envType) {
        ...
        if (sftpHostLabel.isDisplayed()) {
            LogResult.pass("SFTP Host name label is present");
            if (sftpHostTxtBx.isDisplayed()) {

                Object sftpHostName = js.executeScript("return globalState.configHostname");

                LogResult.pass("SFTP Host name text box is displayed with value: " + sftpHostName.toString());
            } else {
                LogResult.fail("SFTP Host name text box is not displayed.");
            }
        } else {
            LogResult.fail("SFTP Host name label is not present.");
        }
    ...
    public void clickOnSettingsSaveBtn() {
        ...
        if(alertCloseBtn.isDisplayed()){
            ...
            if(alertMessage.getText().contentEquals("Updated successfully")){

                LogResult.pass("Changes in Settings updated successfully.");
            }else{
                LogResult.fail("Changes in Settings not updated.");
            }
        ...
        if(alertCloseBtn.isDisplayed()){
            LogResult.pass("Close button is clicked and popup is dismmissed");
        }else{
            LogResult.fail("Popup is not dismissed");
        }

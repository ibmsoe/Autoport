...
class Server(base.APIResourceWrapper):
    ...
    _attrs = ['addresses', 'attrs', 'id', 'image', 'links',
             ...
             'image_name', 'image_vm_mode', 'image_os_type', 'VirtualInterfaces', 'flavor', 'key_name', 'fault',
    ...
    def image_os_type(self):
        ...
        if hasattr(self.image, 'properties'):
            return getattr(self.image, "properties", {}).get("os_type", "-")
        else:
            try:
                image = glance.image_get(self.request, self.image['id'])
                return getattr(image, "properties", {}).get("os_type", "-")
            except glance_exceptions.ClientException:
                return "-"
...
def server_create(request, name, image, flavor, key_name, user_data,
                  ...
                  disk_config=None, config_drive=None, meta=None, accelerator=None):
    return Server(novaclient(request).servers.create(
        ...
        meta=meta, accelerator=accelerator), request)
...
def usage_get(request, tenant_id, start, end, region=None):
    return NovaUsage(novaclient(request, region).usage.get(tenant_id, start, end))
...
def tenant_absolute_limits(request, reserved=False, region=None):
    limits = novaclient(request, region=region).limits.get(reserved=reserved).absolute
    limits_dict = {}
    for limit in limits:
        if limit.value < 0:

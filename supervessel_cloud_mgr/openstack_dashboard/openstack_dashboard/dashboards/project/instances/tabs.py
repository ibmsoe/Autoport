...
from openstack_dashboard.models import WebShellUrl
        ...
        for (key, value) in addresses.items():
            for address in value:
                if 'OS-EXT-IPS:type' in address and address['OS-EXT-IPS:type'] == 'floating':
                    ip = address['addr']
                    break
                if 'type' in address and address['type'] == 'floating':
                    ip = address['addr']
                    break
        try:
            web_url = WebShellUrl.objects.get(ip=ip, region=request.user.services_region)
            console_url = web_url.url
        except Exception, ex:
            console_url = "#"

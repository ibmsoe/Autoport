...
OPENSTACK_FLAVOR_BACKEND = getattr(settings, 'OPENSTACK_FLAVOR_BACKEND', {})
...
def sort_match_flavor_list(request, flavors, pattern):
    """Utility method to sort a list of flavors.
        By default, returns the available flavors, sorted by RAM
        usage (ascending). Override these behaviours with a
        CREATE_INSTANCE_FLAVOR_SORT dict
    ...
    try:
        ...
        for flavor in flavor_list:
            if flavor[1].startswith(pattern):
                find_flavor_list.append(flavor)
                continue
            for key, value in OPENSTACK_FLAVOR_BACKEND.items():
                if email_suffix == key:
                    if flavor[1] in value:
                        find_flavor_list.append(flavor)

        return find_flavor_list
    except Exception:
        exceptions.handle(request,
                          _('Unable to sort instance flavors.'))
        return []

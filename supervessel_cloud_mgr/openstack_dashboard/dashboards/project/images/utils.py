...
def get_available_images_by_filter(request, project_id=None, images_cache=None, filters=None):
    """Returns a list of images that are public or owned by the given
    project_id. If project_id is not specified, only public images
    are returned.
    ...
    if project_id not in images_by_project:
        if filters:
            owner = filters.copy()
            owner.update({"property-owner_id": project_id,
                          "status": "active"})
        else:
            owner = {"property-owner_id": project_id,
                     "status": "active"}

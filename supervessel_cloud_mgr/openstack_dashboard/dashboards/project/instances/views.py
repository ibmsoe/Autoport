...
class IndexView(tables.DataTableView):
    ...
    def get_data(self):
        ...
        if instances:
            ...
            instances = [instance for instance in instances if instance.metadata.get('SPN', '') != 'BigData']
        return instances
...
class LaunchMarketImageView(forms.ModalFormView):
    form_class = project_forms.LaunchMarketImage
    template_name = 'project/instances/launch_market_image.html'
    ...
    def get_initial(self):
        return {"image_id": self.kwargs["image_id"], "image_name": self.get_object().name}

    def get_context_data(self, **kwargs):
        context = super(LaunchMarketImageView, self).get_context_data(**kwargs)
        context['image'] = self.get_object()
        context['flavors'] = json.dumps([f._info for f in

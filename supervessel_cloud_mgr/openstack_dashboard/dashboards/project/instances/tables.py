...
from openstack_dashboard.models import ResourcePoint, WebShellUrl, DELETED
from openstack_dashboard.utils.check_quota import check_instance_quota, QUOTA_EXCEED, NO_BLUE_POINTS
from openstack_dashboard.celery.task.nova import terminate_instance
from openstack_dashboard.celery.task.user_operation import update_operation, recount_resource
...
class TerminateInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        terminate_instance.delay(jsonpickle.encode(request), obj_id)
        update_operation.delay(jsonpickle.encode(request), obj_id, DELETED)
        recount_resource.delay(obj_id, request.user.services_region)
...
class LaunchLink(tables.LinkAction):
    ...
    def allowed(self, request, datum):
        check_quota_ok, status = check_instance_quota(request)
        if not check_quota_ok:
            ...
            self.url = "horizon:project:instances:launch"
...
class StartInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        result = api.nova.server_start(request, obj_id)
...
class InstancesTable(tables.DataTable):
    ...
    os_type = tables.Column("image_os_type",
                            verbose_name=_("OS Type"))
    virt_type = tables.Column("image_vm_mode",
                              verbose_name=_("Virtual Type"),
                              display_choices=VIRT_TYPE_DISPLAY_CHOICES)

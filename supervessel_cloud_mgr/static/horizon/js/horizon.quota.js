...
horizon.Quota = {
  ...
  showFlavorDetails: function() {
    ...
    if (this.selected_flavor) {
      ...
      var flavor_id = this.selected_flavor.id;
      ...
      $("#flavor_ram").text(ram);
        $.get(
            horizon.conf.web_root+'/pointrate?flavor_id='+flavor_id,
            function(data) {
                $("#flavor_points").text(data.points);
            }
        );
    }
  },
  ...
  updateFlavorUsage: function() {
    ...
    $(this.flavor_progress_bars).each(function(index, element) {
      ...
      scope.updateUsageFor(element, update_amount);
        ...
        var quota_new = update_amount + quota_used;
        if (progress_stat == 'instances') {
            $("#quota_instances_text").html(quota_new+'/'+quota_limit);
        } else if (progress_stat == 'vcpus') {
            $("#quota_vcpu_text").html(quota_new+'/'+quota_limit);
        } else if (progress_stat == 'ram') {
            $("#quota_ram_text").html(quota_new+'/'+quota_limit);
        }
    });
  },
  ...
  updateUsageFor: function(progress_element, increment_by) {
    ...
    var percentage_used = ((quota_used / quota_limit) * 100);
     ...
     $(progress_element).attr('aria-valuetransitiongoal', function(){
         return percentage_to_update + percentage_used;
     });
      $('.progress-bar').progressbar({
          display_text : 'fill'
      });
      if ((percentage_to_update + percentage_used) > 70) {
          $(progress_element).attr('class', function() {
              return "progress-bar bg-color-redLight";
          });
      } else {
          $(progress_element).attr('class', function() {
              return "progress-bar bg-color-blue";
          });
      }
